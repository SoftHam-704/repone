...
function FinClienteModal({ initial, onClose, onSaved }: {
  initial: FinCliente | null
  onClose: () => void
  onSaved: () => void
}) {
  const editing = !!initial?.id
  const blank: FinClienteForm = {
    tipo_pessoa: 'J', cpf_cnpj: '', nome_razao: '', nome_fantasia: '',
    endereco: '', numero: '', complemento: '', bairro: '',
    cidade: '', uf: '', cep: '', telefone: '', celular: '',
    email: '', observacoes: '', ativo: true,
  }
  const [form, setForm] = useState<FinClienteForm>(blank)
  const [saving, setSaving] = useState(false)
  const [error, setError]   = useState('')

  useEffect(() => {
    if (initial?.id) {
      api.get(`/financeiro/fin-clientes/${initial.id}`)
        .then(r => r.data.success && setForm({ ...blank, ...r.data.data }))
        .catch(() => {})
    }
  }, [initial?.id])

  const set = (k: keyof FinClienteForm, v: any) => setForm(f => ({ ...f, [k]: v }))
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      if (editing) {
        await api.put(`/financeiro/fin-clientes/${initial!.id}`, form)
      } else {
        await api.post('/financeiro/fin-clientes', form)
      }
      onSaved()
    } catch (err: any) {
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: G.card, border: `1px solid ${G.border}`, borderRadius: 12, padding: 28, width: 640, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,.2)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: G.text }}>{editing ? 'Editar' : 'Novo'} Cliente Financeiro</h3>
...
export default function FinClientesPage() {
  const [items, setItems]     = useState<FinCliente[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch]   = useState('')
  const [modal, setModal]     = useState<{ open: boolean; item: FinCliente | null }>({ open: false, item: null })

  function load() {
    setLoading(true)
    api.get(`/financeiro/fin-clientes?search=${encodeURIComponent(search)}`)
  ...
  async function handleDelete(id: number) {
    if (!confirm('Inativar este cliente financeiro?')) return
    await api.delete(`/financeiro/fin-clientes/${id}`)
    load()
  }
    ...
    <div style={{ background: G.bg, minHeight: '100%' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <Users size={22} color={G.mustard} />
            <div>
              <h1 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: '#FFFFFF' }}>Clientes Financeiros</h1>
              <p style={{ margin: 0, fontSize: 12, color: 'rgba(255,255,255,.5)' }}>Clientes vinculados a contas a receber</p>
            </div>
          </div>
      ...
      <div style={{ padding: '0 28px', marginTop: -28, position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
          {[
            { label: 'Total de Clientes', value: items.length, color: G.navy },
      ...
      <div style={{ padding: '20px 28px 28px' }}>
        ...
        <div style={{ background: G.card, border: `1px solid ${G.border}`, borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,.05)' }}>
          {loading ? (
            ...
            <div style={{ padding: 40, textAlign: 'center', color: G.muted }}>Nenhum cliente financeiro encontrado.</div>
          ) : (
