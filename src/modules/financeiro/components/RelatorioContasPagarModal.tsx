...
export default function RelatorioContasPagarModal({ filters, onClose }: { filters: Filters; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    if (filters.idFornecedor) p.set('idFornecedor', filters.idFornecedor)
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 1200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: G.card, borderRadius: 14, width: '100%', maxWidth: 980, maxHeight: '92vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 64px rgba(0,0,0,.28)', overflow: 'hidden' }}>
        ...
        <div style={{ background: G.navy, color: '#fff', padding: '16px 22px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700 }}>Relatório de Contas a Pagar · por Centro de Custo</h2>
            <p style={{ margin: '2px 0 0', fontSize: 12, color: 'rgba(255,255,255,.6)' }}>Período {periodo} · parcelas pagas em amarelo</p>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => exportRelatorioContasPagar(rows, periodo)} disabled={!rows.length}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, background: G.card, borderRadius: 10, overflow: 'hidden' }}>
              <thead>
                <tr style={{ background: G.navy, color: '#fff' }}>
                  <th style={cL}>Fornecedor</th><th style={cL}>Descrição</th><th style={{ ...cL, textAlign: 'center' }}>Parc.</th>
...
function Grupo({ centro, g }: { centro: string; g: { items: Linha[]; v: number; p: number; s: number } }) {
  return (
    <>
      ...
      {g.items.map(it => {
        ...
        return (
          <tr key={it.numero_parcela + '-' + it.conta_descricao + '-' + it.data_vencimento} style={{ borderTop: `1px solid ${G.border}`, background: paid ? G.yellow : 'transparent' }}>
            <td style={{ ...cL, fontWeight: 500 }}>{it.fornecedor}</td>
