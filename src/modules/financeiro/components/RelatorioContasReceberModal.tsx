...
export default function RelatorioContasReceberModal({ filters, clientes = [], centrosCusto = [], onClose }: {
  filters: Filters
  clientes?: { id: number; nome_razao: string }[]
  ...
  const { grupos, totV, totR, totS } = useMemo(() => {
    ...
    for (const r of rows) {
      const k = (agruparPor === 'centro' ? r.centro_custo : r.cliente) || '(Sem)'
  ...
  const cliNome = f.idCliente ? (clientes.find(x => String(x.id) === f.idCliente)?.nome_razao || '—') : 'Todos'
  const centroNome = f.idCentroCusto ? (centrosCusto.find(x => String(x.id) === f.idCentroCusto)?.descricao || '—') : 'Todos'
  const statusNome = f.status === 'ABERTO' ? 'Em aberto' : f.status === 'RECEBIDO' ? 'Recebidas' : 'Todas'

  const imprimir = () => {
    const linhas: LinhaRel[] = rows.map(r => ({
      centro_custo: r.centro_custo, entidade: r.cliente, conta_descricao: r.conta_descricao,
      numero_documento: r.numero_documento, numero_parcela: r.numero_parcela, data_vencimento: r.data_vencimento,
      valor: Number(r.valor), pago: Number(r.recebido), saldo: Number(r.saldo), status: r.status, paga: r.status === 'RECEBIDO',
    }))
    imprimirRelatorioFinanceiro({
      titulo: 'Relatório de Contas a Receber', entidadeLabel: 'Cliente', valorPagoLabel: 'Recebido',
      statusPagoLabel: 'RECEBIDO', accent: 'receber', empresa, periodo,
      filtros: [`Cliente: ${cliNome}`, `Centro de custo: ${centroNome}`, `Status: ${statusNome}`],
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 1200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: G.card, borderRadius: 14, width: '100%', maxWidth: 1000, maxHeight: '92vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 64px rgba(0,0,0,.28)', overflow: 'hidden' }}>
        ...
        <div style={{ background: G.navy, color: '#fff', padding: '16px 22px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700 }}>Relatório de Contas a Receber</h2>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={() => exportRelatorioContasReceber(rows, periodo, agruparPor)} disabled={!rows.length}
        ...
        <div style={{ padding: '12px 18px', borderBottom: `1px solid ${G.border}`, display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end', background: '#FBF9F4' }}>
          ...
          <label style={lbl}>Cliente
            <select value={f.idCliente} onChange={e => setFil('idCliente', e.target.value)} style={{ ...inp, width: 190 }}>
              <option value="">Todos</option>
              {clientes.map(x => <option key={x.id} value={x.id}>{x.nome_razao}</option>)}
            </select>
          </label>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, background: G.card, borderRadius: 10, overflow: 'hidden' }}>
              <thead>
                <tr style={{ background: G.navy, color: '#fff' }}>
                  <th style={cL}>{agruparPor === 'centro' ? 'Cliente' : 'Centro de Custo'}</th><th style={cL}>Descrição</th><th style={{ ...cL, textAlign: 'center' }}>Parc.</th>
...
function Grupo({ nome, g, agruparPor }: { nome: string; g: { items: Linha[]; v: number; r: number; s: number }; agruparPor: 'centro' | 'entidade' }) {
  const rotulo = agruparPor === 'centro' ? 'CENTRO DE CUSTO' : 'CLIENTE'
  return (
    <>
      ...
      {g.items.map((it, idx) => {
        ...
        const outra = agruparPor === 'centro' ? it.cliente : it.centro_custo
        return (
          <tr key={idx} style={{ borderTop: `1px solid ${G.border}`, background: recebida ? G.yellow : 'transparent' }}>
