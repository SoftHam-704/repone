...
export function useColumnWidths(storageKey: string | undefined, cols: ColW[]) {
  ...
  // NOTA: auto-fit "fit-to-content" (medir com tableLayout:auto) NÃO funciona em
  // grids cujas células são <input> (EditCell) — input tem largura intrínseca
  // grande (~146px) e a coluna incha pro tamanho do input, não do conteúdo.
  // Por isso só expomos `reset` (volta às larguras padrão definidas nas colunas).
  const reset = useCallback(() => setWidths(defaults), [defaults]);

  return { widths, startResize, setWidths, reset };
}
