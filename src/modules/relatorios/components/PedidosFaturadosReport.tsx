...
import { Search, Loader2, AlertCircle, ChevronDown, ChevronRight, CheckSquare, Download } from 'lucide-react';
import * as XLSX from 'xlsx';
import { api } from '@/shared/lib/api';
import { G } from '@/shared/components/layout/CadastroShell';

interface Row { industria_nome: string; ped_pedido: string; cliente: string; ped_data: string; primeira_fat: string; notas: string; valor_pedido: number; valor_faturado: number; ped_situacao: string; }
...
function Section({ nome, rows, color }: { nome: string; rows: Row[]; color: string }) {
  ...
  const subtFat = rows.reduce((s, r) => s + n(r.valor_faturado), 0);
  return (
    <div style={{ borderRadius: 12, overflow: 'hidden', border: `1px solid ${G.border}`, marginBottom: 8 }}>
      <button onClick={() => setOpen(o => !o)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 12, padding: '11px 16px', background: open ? `${color}12` : G.card, border: 'none', cursor: 'pointer', textAlign: 'left', borderBottom: open ? `1px solid ${G.border}` : 'none', transition: 'background 0.15s' }}>
        <div style={{ width: 32, height: 32, borderRadius: 9, flexShrink: 0, background: `${color}20`, border: `1px solid ${color}40`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><CheckSquare size={14} style={{ color }} /></div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 12, fontWeight: 800, color: open ? color : G.text }}>{nome}</div>
          <div style={{ fontSize: 10, color: G.textMuted, marginTop: 1 }}>{rows.length} pedido{rows.length !== 1 ? 's' : ''}</div>
        </div>
        <div style={{ display: 'flex', gap: 28, marginRight: 8 }}>
          ...
          <div style={{ textAlign: 'right' }}><div style={{ fontSize: 10, color: G.textMuted, fontWeight: 600 }}>Vlr. Faturado</div><div style={{ fontSize: 13, fontWeight: 800, color: '#16A34A' }}>{fmtBRL(subtFat)}</div></div>
        </div>
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.18 }} style={{ overflow: 'hidden' }}>
            <table style={{ borderCollapse: 'collapse', width: '100%' }}>
              <thead><tr style={{ background: G.cardHi }}>
                {['Pedido','Dt. Pedido','Data Fat.','Notas','Cliente','Vlr. Pedido','Vlr. Faturado','Sit.'].map((h, i) => (
                  <th key={h} style={{ ...CELL, textAlign: i >= 5 ? 'right' : i === 7 ? 'center' : 'left', color: G.textMuted, fontWeight: 700, borderBottom: `2px solid ${G.border}`, fontSize: 12 }}>{h}</th>
                ))}
              </tr></thead>
              <tbody>
                {rows.map((r, i) => {
                  const { label, bg, color: bc } = badgeSit(r.ped_situacao);
                  return (
                    <tr key={`${r.ped_pedido}-${i}`} style={{ background: i % 2 === 0 ? G.card : G.bg }}>
                      ...
                      <td style={{ ...CELL, color: G.textMuted }}>{fmtDate(r.primeira_fat)}</td>
                      <td style={{ ...CELL, fontFamily: 'monospace', fontSize: 11, color: G.textMuted }}>{r.notas}</td>
                      ...
                      <td style={{ ...CELL, textAlign: 'right', fontFamily: 'monospace', fontWeight: 700, color: '#16A34A' }}>{fmtBRL(r.valor_faturado)}</td>
                      <td style={{ ...CELL, textAlign: 'center' }}>
                        <span style={{ fontSize: 10, fontWeight: 800, padding: '2px 7px', borderRadius: 4, background: bg, color: bc }}>{label}</span>
                      </td>
                    </tr>
                  );
                })}
              ...
              <tfoot><tr style={{ background: `${color}10` }}>
                <td colSpan={5} style={{ ...CELL, fontWeight: 800, fontSize: 12, color, textTransform: 'uppercase' }}>Subtotal {nome}</td>
                <td style={{ ...CELL, textAlign: 'right', fontFamily: 'monospace', fontWeight: 700, color: G.textSec }}>{fmtBRL(subtPed)}</td>
                <td style={{ ...CELL, textAlign: 'right', fontFamily: 'monospace', fontWeight: 800, color: '#16A34A' }}>{fmtBRL(subtFat)}</td>
                <td />
              </tr></tfoot>
            </table>
          </motion.div>
...
export default function PedidosFaturadosReport() {
  ...
  const visualizar = useCallback(async () => {
    ...
    try {
      ...
      const r = await api.get(`/reports/faturamento/pedidos-faturados?${p}`);
  ...
  const handleExport = () => {
    ...
    for (const nome of nomes) {
      ...
      const ws = XLSX.utils.aoa_to_sheet([
        ['Pedido','Dt. Pedido','Data Fat.','Notas','Cliente','Vlr. Pedido','Vlr. Faturado','Situação'],
        ...linhas.map(r => [r.ped_pedido, fmtDate(r.ped_data), fmtDate(r.primeira_fat), r.notas, r.cliente, n(r.valor_pedido), n(r.valor_faturado), badgeSit(r.ped_situacao).label]),
        ['','','','','SUBTOTAL', linhas.reduce((s,r)=>s+n(r.valor_pedido),0), linhas.reduce((s,r)=>s+n(r.valor_faturado),0), ''],
      ]);
      ws['!cols'] = [14,12,12,14,28,14,14,12].map(w => ({ wch: w }));
      XLSX.utils.book_append_sheet(wb, ws, nome.substring(0, 31));
    }
    XLSX.writeFile(wb, `pedidos_faturados_${dataInicio}_${dataFim}.xlsx`);
  };
  ...
  const grandFat = rows.reduce((s, r) => s + n(r.valor_faturado), 0);

  return (
    <div style={{ padding: '20px 24px', background: G.bg, minHeight: '100%', display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', gap: 12 }}>
        {[['Data Fat. Inicial', dataInicio, setDataInicio], ['Data Fat. Final', dataFim, setDataFim]].map(([lbl, val, set]) => (
      ...
      {gerado && rows.length > 0 && (
        <div style={{ display: 'flex', gap: 12 }}>
          {[{ label: 'Pedidos', val: rows.length.toLocaleString('pt-BR'), color: '#0891B2' }, { label: 'Indústrias', val: String(grupos.length), color: '#7C3AED' }, { label: 'Vlr. Pedidos', val: fmtBRL(grandPed), color: G.mustard }, { label: 'Vlr. Faturado', val: fmtBRL(grandFat), color: '#16A34A' }].map(k => (
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ background: NAVY_DRK, borderRadius: 12, padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 }}>
            ...
            <div style={{ display: 'flex', gap: 32 }}>
              <div style={{ textAlign: 'right' }}><div style={{ fontSize: 10, color: '#94A3B8', fontWeight: 600 }}>Vlr. Pedidos</div><div style={{ fontSize: 13, fontWeight: 700, color: '#94A3B8', fontFamily: 'monospace' }}>{fmtBRL(grandPed)}</div></div>
              <div style={{ textAlign: 'right' }}><div style={{ fontSize: 10, color: '#94A3B8', fontWeight: 600 }}>Vlr. Faturado</div><div style={{ fontSize: 15, fontWeight: 800, color: '#FCD34D', fontFamily: 'monospace' }}>{fmtBRL(grandFat)}</div></div>
            </div>
          </div>
        </div>
      )}
      {gerado && rows.length === 0 && !error && <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12, color: G.textMuted }}><CheckSquare size={40} style={{ opacity: 0.3 }} /><span style={{ fontSize: 13, fontWeight: 600 }}>Nenhum pedido faturado no período</span></div>}
