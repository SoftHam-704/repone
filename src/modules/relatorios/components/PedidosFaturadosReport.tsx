...
export default function PedidosFaturadosReport() {
  ...
  const visualizar = useCallback(async () => {
    ...
    try {
      ...
      const r = await api.get(`/reports/faturamento/pedidos-faturados?${params}`);
  ...
  const handlePrint = () => {
    ...
    if (!w) return;
    w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Pedidos Faturados no Período</title>
    <style>
  ...
  const totalGeral = rows.reduce((a, r) => ({ fat: a.fat + (r.valor_faturado || 0), n: a.n + 1 }), { fat: 0, n: 0 });

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'flex-end' }}>
        <div>
          <label style={{ fontSize: 10, fontWeight: 700, color: G.textMuted, display: 'block', marginBottom: 4 }}>DATA FAT. INICIAL *</label>
          <input type="date" style={inp} value={dataInicio} onChange={e => setDataInicio(e.target.value)} />
        </div>
        <div>
          <label style={{ fontSize: 10, fontWeight: 700, color: G.textMuted, display: 'block', marginBottom: 4 }}>DATA FAT. FINAL *</label>
        ...
        <div ref={printRef}>
          {empresa && (
            <div className="company-box" style={{ border: `1px solid ${G.border}`, borderRadius: 10, padding: '12px 18px', textAlign: 'center', marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 11, color: G.textMuted, marginTop: 6 }}>Período de faturamento: {fmtDate(dataInicio)} até {fmtDate(dataFim)}</div>
            </div>
          )}
          <h2 style={{ fontSize: 14, fontWeight: 900, color: G.text, textAlign: 'center', marginBottom: 14 }}>Pedidos Faturados no Período</h2>

          {rows.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 40, color: G.textMuted, fontSize: 13 }}>Nenhum pedido faturado no período.</div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  {[
                    ...
                    ['Data Fat.', 'right'], ['Nota(s)', 'left'], ['Valor Pedido', 'right'],
                    ['Valor Faturado', 'right'], ['Sit.', 'center']
                  ].map(([h, a]) => (
              ...
              <tbody>
                {Object.entries(grupos).map(([nome, linhas]) => {
                  const sub = linhas.reduce((a, r) => ({ fat: a.fat + (r.valor_faturado || 0), n: a.n + 1 }), { fat: 0, n: 0 });
                  return (
                    <Fragment key={`group-${nome}`}>
                      ...
                      {linhas.map(r => {
                        const { label, bg, color } = badgeSit(r.ped_situacao);
                        return (
                          <tr key={`${nome}-${r.ped_pedido}`} style={{ background: 'transparent' }}>
                            ...
                            <td style={{ ...td, textAlign: 'right' }}>{fmtDate(r.primeira_fat)}</td>
                            <td style={{ ...td, fontFamily: 'monospace', fontSize: 10 }}>{r.notas}</td>
                            <td style={{ ...td, textAlign: 'right' }}>{fmtBRL(r.valor_pedido)}</td>
                            <td style={{ ...td, textAlign: 'right', fontWeight: 700 }}>{fmtBRL(r.valor_faturado)}</td>
                            <td style={{ ...td, textAlign: 'center' }}>
                              <span style={{ fontSize: 9, fontWeight: 800, padding: '2px 6px', borderRadius: 4, background: bg, color }}>{label}</span>
                            </td>
                          </tr>
                        );
                      })}
                      <tr style={{ background: `${COLOR}12`, borderTop: `1px solid ${COLOR}40` }}>
                        <td colSpan={5} style={{ ...td, textAlign: 'right', fontWeight: 800, fontSize: 11, color: G.textSec }}>{sub.n} pedido(s)</td>
                        <td />
                ...
                <tr style={{ background: G.text }}>
                  <td colSpan={5} style={{ ...td, color: G.mustard, fontWeight: 900, textAlign: 'right' }}>{totalGeral.n} pedidos</td>
                  <td />
