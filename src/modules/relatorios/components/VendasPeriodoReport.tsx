...
export default function VendasPeriodoReport() {
  ...
  const visualizar = useCallback(async () => {
    ...
    try {
      ...
      const r = await api.get(`/reports/vendas-periodo?${params}`);
  ...
  }, [dataInicio, dataFim, selVendedor, selIndustria]);
  ...
  const vendLabel    = vendedores.find(v => String(v.ven_codigo) === selVendedor)?.ven_nome  ?? 'Todos os vendedores';
      ...
      {gerado && (
        <div ref={printRef}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 12 }}>
            <h2 style={{ fontSize: 14, fontWeight: 900, color: G.text }}>Vendas no Período</h2>
            <p style={{ fontSize: 11, color: G.textMuted, marginTop: 2 }}>
              {fmtDate(dataInicio)} a {fmtDate(dataFim)} · {vendLabel} · {indLabel}
            </p>
          </div>
                    ...
                    <>
                      ...
                      {linhas.map((r, i) => (
                        <tr key={`${r.ped_pedido}-${i}`} style={{ background: i % 2 === 0 ? G.card : G.cardHi }}>
                          ...
                          <td style={{ ...td, maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.cli_nomred}</td>
