...
import { Search, Loader2, AlertCircle, ChevronDown, ChevronRight, FileText, Download } from 'lucide-react';
import * as XLSX from 'xlsx';
import { api } from '@/shared/lib/api';
import { G } from '@/shared/components/layout/CadastroShell';

interface Row { industria_nome: string; cliente: string; ped_pedido: string; fat_datafat: string; fat_nf: string; fat_valorfat: number; percent_rep: number; comissao_rep: number; }
...
function Section({ nome, rows, color }: { nome: string; rows: Row[]; color: string }) {
  ...
  const subtCom = rows.reduce((s, r) => s + n(r.comissao_rep), 0);
  return (
    <div style={{ borderRadius: 12, overflow: 'hidden', border: `1px solid ${G.border}`, marginBottom: 8 }}>
      <button onClick={() => setOpen(o => !o)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 12, padding: '11px 16px', background: open ? `${color}12` : G.card, border: 'none', cursor: 'pointer', textAlign: 'left', borderBottom: open ? `1px solid ${G.border}` : 'none', transition: 'background 0.15s' }}>
        <div style={{ width: 32, height: 32, borderRadius: 9, flexShrink: 0, background: `${color}20`, border: `1px solid ${color}40`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><FileText size={14} style={{ color }} /></div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 12, fontWeight: 800, color: open ? color : G.text }}>{nome}</div>
          <div style={{ fontSize: 10, color: G.textMuted, marginTop: 1 }}>{rows.length} nota{rows.length !== 1 ? 's' : ''}</div>
        </div>
        <div style={{ display: 'flex', gap: 28, marginRight: 8 }}>
          <div style={{ textAlign: 'right' }}><div style={{ fontSize: 10, color: G.textMuted, fontWeight: 600 }}>Faturado</div><div style={{ fontSize: 12, fontWeight: 700, color: G.textSec }}>{fmtBRL(subtFat)}</div></div>
          <div style={{ textAlign: 'right' }}><div style={{ fontSize: 10, color: G.textMuted, fontWeight: 600 }}>Comissão</div><div style={{ fontSize: 13, fontWeight: 800, color: '#D97706' }}>{fmtBRL(subtCom)}</div></div>
        </div>
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.18 }} style={{ overflow: 'hidden' }}>
            <table style={{ borderCollapse: 'collapse', width: '100%' }}>
              <thead><tr style={{ background: G.cardHi }}>
                {['Pedido','Data Fat.','NF','Cliente','Vlr. Faturado','%','Comissão'].map((h, i) => (
              ...
              <tbody>
                {rows.map((r, i) => (
                  <tr key={`${r.ped_pedido}-${r.fat_nf}`} style={{ background: i % 2 === 0 ? G.card : G.bg }}>
                    ...
                    <td style={{ ...CELL, textAlign: 'right', fontFamily: 'monospace', fontWeight: 600, color: G.textSec }}>{fmtBRL(r.fat_valorfat)}</td>
                    <td style={{ ...CELL, textAlign: 'right', color: G.textMuted }}>{fmtPct(r.percent_rep)}</td>
                    <td style={{ ...CELL, textAlign: 'right', fontFamily: 'monospace', fontWeight: 700, color: '#D97706' }}>{fmtBRL(r.comissao_rep)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot><tr style={{ background: `${color}10` }}>
                <td colSpan={4} style={{ ...CELL, fontWeight: 800, fontSize: 12, color, textTransform: 'uppercase' }}>Subtotal {nome}</td>
                <td style={{ ...CELL, textAlign: 'right', fontFamily: 'monospace', fontWeight: 700, color: G.textSec }}>{fmtBRL(subtFat)}</td>
                <td />
                <td style={{ ...CELL, textAlign: 'right', fontFamily: 'monospace', fontWeight: 800, color: '#D97706' }}>{fmtBRL(subtCom)}</td>
              </tr></tfoot>
            </table>
          </motion.div>
        )}
...
export default function FaturamentoPeriodoReport() {
  ...
  const visualizar = useCallback(async () => {
    ...
    try {
      ...
      const r = await api.get(`/reports/faturamento/periodo?${p}`);
  ...
  const handleExport = () => {
    ...
    for (const nome of grupos) {
      ...
      const ws = XLSX.utils.aoa_to_sheet([
        ['Pedido','Data Fat.','NF','Cliente','Vlr. Faturado','%','Comissão'],
        ...linhas.map(r => [r.ped_pedido, fmtDate(r.fat_datafat), r.fat_nf, r.cliente, n(r.fat_valorfat), n(r.percent_rep), n(r.comissao_rep)]),
        ['','','','SUBTOTAL', linhas.reduce((s,r)=>s+n(r.fat_valorfat),0), '', linhas.reduce((s,r)=>s+n(r.comissao_rep),0)],
      ]);
      ws['!cols'] = [14,12,10,28,14,8,14].map(w => ({ wch: w }));
      XLSX.utils.book_append_sheet(wb, ws, nome.substring(0, 31));
    }
    XLSX.writeFile(wb, `faturamento_periodo_${dataInicio}_${dataFim}.xlsx`);
  };
  ...
  const grandCom = rows.reduce((s, r) => s + n(r.comissao_rep), 0);

  return (
    <div style={{ padding: '20px 24px', background: G.bg, minHeight: '100%', display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', gap: 12 }}>
        {[['Data Inicial', dataInicio, setDataInicio], ['Data Final', dataFim, setDataFim]].map(([lbl, val, set]) => (
      ...
      {gerado && rows.length > 0 && (
        <div style={{ display: 'flex', gap: 12 }}>
          {[{ label: 'Notas', val: rows.length.toLocaleString('pt-BR'), color: '#0891B2' }, { label: 'Indústrias', val: String(grupos.length), color: '#7C3AED' }, { label: 'Vlr. Faturado', val: fmtBRL(grandFat), color: G.mustard }, { label: 'Comissão Rep.', val: fmtBRL(grandCom), color: '#D97706' }].map(k => (
      ...
      {gerado && grupos.length > 0 && (
        <div style={{ flex: 1 }}>
          ...
          <div style={{ background: NAVY_DRK, borderRadius: 12, padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 }}>
            <span style={{ fontSize: 13, fontWeight: 800, color: G.mustard, textTransform: 'uppercase', letterSpacing: 0.5 }}>TOTAL GERAL — {rows.length} nota{rows.length !== 1 ? 's' : ''} · {grupos.length} indústria{grupos.length !== 1 ? 's' : ''}</span>
            <div style={{ display: 'flex', gap: 32 }}>
              <div style={{ textAlign: 'right' }}><div style={{ fontSize: 10, color: '#94A3B8', fontWeight: 600 }}>Vlr. Faturado</div><div style={{ fontSize: 13, fontWeight: 700, color: '#94A3B8', fontFamily: 'monospace' }}>{fmtBRL(grandFat)}</div></div>
              <div style={{ textAlign: 'right' }}><div style={{ fontSize: 10, color: '#94A3B8', fontWeight: 600 }}>Comissão</div><div style={{ fontSize: 15, fontWeight: 800, color: '#FCD34D', fontFamily: 'monospace' }}>{fmtBRL(grandCom)}</div></div>
            </div>
          </div>
        </div>
      )}
      {gerado && rows.length === 0 && !error && <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12, color: G.textMuted }}><FileText size={40} style={{ opacity: 0.3 }} /><span style={{ fontSize: 13, fontWeight: 600 }}>Nenhum faturamento no período</span></div>}
