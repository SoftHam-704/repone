...
export default function ProdutosNaoFaturadosReport() {
  ...
  const visualizar = useCallback(async () => {
    ...
    try {
      ...
      const r = await api.get(`/reports/faturamento/produtos-nao-faturados?${params}`);
  ...
  const handlePrint = () => {
    ...
    if (!w) return;
    w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Produtos Não Faturados</title>
    <style>
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      td { padding: 4px 7px; border-bottom: 1px solid #e5e0d5; }
      td.r { text-align: right; }
      .cod { font-family: monospace; font-weight: 900; font-size: 12px; }
  ...
  const totalGeral = rows.reduce((a, r) => ({ saldo: a.saldo + (r.saldo || 0), n: a.n + 1 }), { saldo: 0, n: 0 });

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'flex-end' }}>
        <div>
          <label style={{ fontSize: 10, fontWeight: 700, color: G.textMuted, display: 'block', marginBottom: 4 }}>DATA PEDIDO INICIAL *</label>
          <input type="date" style={inp} value={dataInicio} onChange={e => setDataInicio(e.target.value)} />
        </div>
        <div>
          <label style={{ fontSize: 10, fontWeight: 700, color: G.textMuted, display: 'block', marginBottom: 4 }}>DATA PEDIDO FINAL *</label>
        ...
        <div ref={printRef}>
          {empresa && (
            <div className="company-box" style={{ border: `1px solid ${G.border}`, borderRadius: 10, padding: '12px 18px', textAlign: 'center', marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 11, color: G.textMuted, marginTop: 6 }}>Pedidos de {fmtDate(dataInicio)} até {fmtDate(dataFim)}</div>
            </div>
          )}
          <h2 style={{ fontSize: 14, fontWeight: 900, color: G.text, textAlign: 'center', marginBottom: 14 }}>Produtos Não Faturados</h2>

          {rows.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 40, color: G.textMuted, fontSize: 13 }}>Nenhum produto pendente de faturamento no período.</div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  {[
                    ['Pedido', 'left'], ['Dt. Pedido', 'right'], ['Cliente', 'left'],
                    ['Código', 'left'], ['Produto', 'left'],
                    ['Qtd. Vend.', 'right'], ['Qtd. Fat.', 'right'], ['Saldo', 'right']
                  ].map(([h, a]) => (
              ...
              <tbody>
                {Object.entries(grupos).map(([nome, linhas]) => {
                  const sub = linhas.reduce((a, r) => ({ saldo: a.saldo + (r.saldo || 0) }), { saldo: 0 });
                  return (
                    <Fragment key={`group-${nome}`}>
                      <tr style={{ background: `${COLOR}10`, borderTop: `2px solid ${COLOR}` }}>
                        <td colSpan={8} style={{ ...td, fontWeight: 900, fontSize: 12, color: G.text }}>{nome}</td>
                      </tr>
                      {linhas.map((r, i) => (
                        <tr key={`${nome}-${r.ped_pedido}-${r.codigo}-${i}`}>
                          ...
                          <td style={{ ...td }}>
                            <span style={{ fontFamily: 'monospace', fontWeight: 900, fontSize: 12 }}>{r.codigo}</span>
                          </td>
                          <td style={{ ...td, fontSize: 11, color: G.textSec }}>{r.produto}</td>
                          <td style={{ ...td, textAlign: 'right' }}>{r.qtd_vendida}</td>
                          <td style={{ ...td, textAlign: 'right', color: r.qtd_faturada > 0 ? '#065F46' : G.textMuted }}>{r.qtd_faturada}</td>
                          <td style={{ ...td, textAlign: 'right', fontWeight: 900, color: '#B45309' }}>{r.saldo}</td>
                        </tr>
                      ))}
                      <tr style={{ background: `${COLOR}12`, borderTop: `1px solid ${COLOR}40` }}>
                        <td colSpan={7} style={{ ...td, textAlign: 'right', fontWeight: 800, fontSize: 11, color: G.textSec }}>{linhas.length} item(ns) pendente(s)</td>
                        <td style={{ ...td, textAlign: 'right', fontWeight: 900 }}>{sub.saldo}</td>
                      </tr>
                    </Fragment>
                  );
                })}
                <tr style={{ background: G.text }}>
                  <td colSpan={7} style={{ ...td, color: G.mustard, fontWeight: 900, textAlign: 'right' }}>{totalGeral.n} itens — saldo total</td>
                  <td style={{ ...td, textAlign: 'right', fontWeight: 900, fontSize: 13, color: G.mustard }}>{totalGeral.saldo}</td>
                </tr>
              </tbody>
            </table>
          )}
