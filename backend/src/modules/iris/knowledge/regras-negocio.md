...
- **Como o progresso é apurado (base de apuração):** cada campanha grava sua **base** (`cmp_base_apuracao`): **SELL-OUT** = mede o que o lojista **vendeu** na ponta (via `crm_sellout`, o giro real) · **SELL-IN** = o que ele **comprou** (pedidos P/F). Campanha nova nasce **sell-out quando o cliente reporta sell-out**; sem reporte, cai em **sell-in (proxy)** com selo "giro não confirmado" e estado "aguardando reporte" (não é 0%). **MIX é sempre sell-in** (o sell-out atual não tem quebra por família). A tela do monitoramento mostra sempre o trio **Comprou (sell-in) · Vendeu (sell-out) · Giro (sell-through%)** — sell-through baixo denuncia estoque empurrado. Campanhas já firmadas (ATIVA/CONCLUÍDA) mantêm a base com que foram fechadas.
