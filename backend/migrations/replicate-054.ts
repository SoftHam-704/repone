...
// Replica a migration 054 (for_email → TEXT) em todos os schemas de tenant do DB basesales.
...
async function main() {
  ...
  const sql = fs.readFileSync(path.join(__dirname, '054_fornecedores_email_text.sql'), 'utf8');
  ...
  const schemas = (await pool.query(`
    ...
    WHERE schema_name NOT IN ('public','pg_catalog','information_schema','pg_toast','basesales')
      ...
      AND EXISTS (
        ...
        WHERE t.table_schema = schema_name AND t.table_name = 'fornecedores'
      )
    ORDER BY schema_name
  `)).rows.map(r => r.schema_name);

  console.log(`Migration 054 → ${schemas.length} schemas com fornecedores...`);
  let ok = 0, fail = 0;
