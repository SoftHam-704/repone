...
// Replica a migration 052 nos schemas de tenant que têm fin_nfse.
async function main() {
  ...
  const sql = fs.readFileSync(path.join(__dirname, '052_fin_nfse_protocolo_text.sql'), 'utf8');
  ...
  const schemas = (await pool.query(`
    ...
    WHERE schema_name NOT IN ('public','pg_catalog','information_schema','pg_toast','basesales')
      ...
      AND EXISTS (
        ...
        WHERE t.table_schema = schema_name AND t.table_name = 'fin_nfse'
      )
    ORDER BY schema_name
  `)).rows.map(r => r.schema_name);

  console.log(`Migration 052 → ${schemas.length} schemas com fin_nfse...`);
  let ok = 0, fail = 0;
